...
function opj_version()
    ccall((:opj_version, libopenjpegextra), Ptr{Cchar}, ())
end

function opj_image_create(numcmpts, cmptparms, clrspc)
    ccall((:opj_image_create, libopenjpegextra), Ptr{opj_image_t}, (OPJ_UINT32, Ptr{opj_image_cmptparm_t}, OPJ_COLOR_SPACE), numcmpts, cmptparms, clrspc)
end

function opj_image_destroy(image)
    ccall((:opj_image_destroy, libopenjpegextra), Cvoid, (Ptr{opj_image_t},), image)
end

function opj_image_tile_create(numcmpts, cmptparms, clrspc)
    ccall((:opj_image_tile_create, libopenjpegextra), Ptr{opj_image_t}, (OPJ_UINT32, Ptr{opj_image_cmptparm_t}, OPJ_COLOR_SPACE), numcmpts, cmptparms, clrspc)
end

function opj_image_data_alloc(size)
    ccall((:opj_image_data_alloc, libopenjpegextra), Ptr{Cvoid}, (OPJ_SIZE_T,), size)
end

function opj_image_data_free(ptr)
    ccall((:opj_image_data_free, libopenjpegextra), Cvoid, (Ptr{Cvoid},), ptr)
end

function opj_stream_default_create(p_is_input)
    ccall((:opj_stream_default_create, libopenjpegextra), Ptr{opj_stream_t}, (OPJ_BOOL,), p_is_input)
end

function opj_stream_create(p_buffer_size, p_is_input)
    ccall((:opj_stream_create, libopenjpegextra), Ptr{opj_stream_t}, (OPJ_SIZE_T, OPJ_BOOL), p_buffer_size, p_is_input)
end

function opj_stream_destroy(p_stream)
    ccall((:opj_stream_destroy, libopenjpegextra), Cvoid, (Ptr{opj_stream_t},), p_stream)
end

function opj_stream_set_read_function(p_stream, p_function)
    ccall((:opj_stream_set_read_function, libopenjpegextra), Cvoid, (Ptr{opj_stream_t}, opj_stream_read_fn), p_stream, p_function)
end

function opj_stream_set_write_function(p_stream, p_function)
    ccall((:opj_stream_set_write_function, libopenjpegextra), Cvoid, (Ptr{opj_stream_t}, opj_stream_write_fn), p_stream, p_function)
end

function opj_stream_set_skip_function(p_stream, p_function)
    ccall((:opj_stream_set_skip_function, libopenjpegextra), Cvoid, (Ptr{opj_stream_t}, opj_stream_skip_fn), p_stream, p_function)
end

function opj_stream_set_seek_function(p_stream, p_function)
    ccall((:opj_stream_set_seek_function, libopenjpegextra), Cvoid, (Ptr{opj_stream_t}, opj_stream_seek_fn), p_stream, p_function)
end

function opj_stream_set_user_data(p_stream, p_data, p_function)
    ccall((:opj_stream_set_user_data, libopenjpegextra), Cvoid, (Ptr{opj_stream_t}, Ptr{Cvoid}, opj_stream_free_user_data_fn), p_stream, p_data, p_function)
end

function opj_stream_set_user_data_length(p_stream, data_length)
    ccall((:opj_stream_set_user_data_length, libopenjpegextra), Cvoid, (Ptr{opj_stream_t}, OPJ_UINT64), p_stream, data_length)
end

function opj_stream_create_default_file_stream(fname, p_is_read_stream)
    ccall((:opj_stream_create_default_file_stream, libopenjpegextra), Ptr{opj_stream_t}, (Ptr{Cchar}, OPJ_BOOL), fname, p_is_read_stream)
end

function opj_stream_create_file_stream(fname, p_buffer_size, p_is_read_stream)
    ccall((:opj_stream_create_file_stream, libopenjpegextra), Ptr{opj_stream_t}, (Ptr{Cchar}, OPJ_SIZE_T, OPJ_BOOL), fname, p_buffer_size, p_is_read_stream)
end

function opj_set_info_handler(p_codec, p_callback, p_user_data)
    ccall((:opj_set_info_handler, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, opj_msg_callback, Ptr{Cvoid}), p_codec, p_callback, p_user_data)
end

function opj_set_warning_handler(p_codec, p_callback, p_user_data)
    ccall((:opj_set_warning_handler, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, opj_msg_callback, Ptr{Cvoid}), p_codec, p_callback, p_user_data)
end

function opj_set_error_handler(p_codec, p_callback, p_user_data)
    ccall((:opj_set_error_handler, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, opj_msg_callback, Ptr{Cvoid}), p_codec, p_callback, p_user_data)
end

function opj_create_decompress(format)
    ccall((:opj_create_decompress, libopenjpegextra), Ptr{opj_codec_t}, (OPJ_CODEC_FORMAT,), format)
end

function opj_destroy_codec(p_codec)
    ccall((:opj_destroy_codec, libopenjpegextra), Cvoid, (Ptr{opj_codec_t},), p_codec)
end

function opj_end_decompress(p_codec, p_stream)
    ccall((:opj_end_decompress, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_stream_t}), p_codec, p_stream)
end

function opj_set_default_decoder_parameters(parameters)
    ccall((:opj_set_default_decoder_parameters, libopenjpegextra), Cvoid, (Ptr{opj_dparameters_t},), parameters)
end

function opj_setup_decoder(p_codec, parameters)
    ccall((:opj_setup_decoder, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_dparameters_t}), p_codec, parameters)
end

function opj_codec_set_threads(p_codec, num_threads)
    ccall((:opj_codec_set_threads, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Cint), p_codec, num_threads)
end

function opj_read_header(p_stream, p_codec, p_image)
    ccall((:opj_read_header, libopenjpegextra), OPJ_BOOL, (Ptr{opj_stream_t}, Ptr{opj_codec_t}, Ptr{Ptr{opj_image_t}}), p_stream, p_codec, p_image)
end

function opj_set_decoded_components(p_codec, numcomps, comps_indices, apply_color_transforms)
    ccall((:opj_set_decoded_components, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, OPJ_UINT32, Ptr{OPJ_UINT32}, OPJ_BOOL), p_codec, numcomps, comps_indices, apply_color_transforms)
end

function opj_set_decode_area(p_codec, p_image, p_start_x, p_start_y, p_end_x, p_end_y)
    ccall((:opj_set_decode_area, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_image_t}, OPJ_INT32, OPJ_INT32, OPJ_INT32, OPJ_INT32), p_codec, p_image, p_start_x, p_start_y, p_end_x, p_end_y)
end

function opj_decode(p_decompressor, p_stream, p_image)
    ccall((:opj_decode, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_stream_t}, Ptr{opj_image_t}), p_decompressor, p_stream, p_image)
end

function opj_get_decoded_tile(p_codec, p_stream, p_image, tile_index)
    ccall((:opj_get_decoded_tile, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_stream_t}, Ptr{opj_image_t}, OPJ_UINT32), p_codec, p_stream, p_image, tile_index)
end

function opj_set_decoded_resolution_factor(p_codec, res_factor)
    ccall((:opj_set_decoded_resolution_factor, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, OPJ_UINT32), p_codec, res_factor)
end

function opj_write_tile(p_codec, p_tile_index, p_data, p_data_size, p_stream)
    ccall((:opj_write_tile, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, OPJ_UINT32, Ptr{OPJ_BYTE}, OPJ_UINT32, Ptr{opj_stream_t}), p_codec, p_tile_index, p_data, p_data_size, p_stream)
end
...
function opj_read_tile_header(p_codec, p_stream, p_tile_index, p_data_size, p_tile_x0, p_tile_y0, p_tile_x1, p_tile_y1, p_nb_comps, p_should_go_on)
    ccall((:opj_read_tile_header, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_stream_t}, Ptr{OPJ_UINT32}, Ptr{OPJ_UINT32}, Ptr{OPJ_INT32}, Ptr{OPJ_INT32}, Ptr{OPJ_INT32}, Ptr{OPJ_INT32}, Ptr{OPJ_UINT32}, Ptr{OPJ_BOOL}), p_codec, p_stream, p_tile_index, p_data_size, p_tile_x0, p_tile_y0, p_tile_x1, p_tile_y1, p_nb_comps, p_should_go_on)
end

function opj_decode_tile_data(p_codec, p_tile_index, p_data, p_data_size, p_stream)
    ccall((:opj_decode_tile_data, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, OPJ_UINT32, Ptr{OPJ_BYTE}, OPJ_UINT32, Ptr{opj_stream_t}), p_codec, p_tile_index, p_data, p_data_size, p_stream)
end

function opj_create_compress(format)
    ccall((:opj_create_compress, libopenjpegextra), Ptr{opj_codec_t}, (OPJ_CODEC_FORMAT,), format)
end

function opj_set_default_encoder_parameters(parameters)
    ccall((:opj_set_default_encoder_parameters, libopenjpegextra), Cvoid, (Ptr{opj_cparameters_t},), parameters)
end

function opj_setup_encoder(p_codec, parameters, image)
    ccall((:opj_setup_encoder, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_cparameters_t}, Ptr{opj_image_t}), p_codec, parameters, image)
end

function opj_encoder_set_extra_options(p_codec, p_options)
    ccall((:opj_encoder_set_extra_options, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{Ptr{Cchar}}), p_codec, p_options)
end

function opj_start_compress(p_codec, p_image, p_stream)
    ccall((:opj_start_compress, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_image_t}, Ptr{opj_stream_t}), p_codec, p_image, p_stream)
end

function opj_end_compress(p_codec, p_stream)
    ccall((:opj_end_compress, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_stream_t}), p_codec, p_stream)
end

function opj_encode(p_codec, p_stream)
    ccall((:opj_encode, libopenjpegextra), OPJ_BOOL, (Ptr{opj_codec_t}, Ptr{opj_stream_t}), p_codec, p_stream)
end

function opj_destroy_cstr_info(cstr_info)
    ccall((:opj_destroy_cstr_info, libopenjpegextra), Cvoid, (Ptr{Ptr{opj_codestream_info_v2_t}},), cstr_info)
end

function opj_dump_codec(p_codec, info_flag, output_stream)
    ccall((:opj_dump_codec, libopenjpegextra), Cvoid, (Ptr{opj_codec_t}, OPJ_INT32, Ptr{Libc.FILE}), p_codec, info_flag, output_stream)
end

function opj_get_cstr_info(p_codec)
    ccall((:opj_get_cstr_info, libopenjpegextra), Ptr{opj_codestream_info_v2_t}, (Ptr{opj_codec_t},), p_codec)
end

function opj_get_cstr_index(p_codec)
    ccall((:opj_get_cstr_index, libopenjpegextra), Ptr{opj_codestream_index_t}, (Ptr{opj_codec_t},), p_codec)
end

function opj_destroy_cstr_index(p_cstr_index)
    ccall((:opj_destroy_cstr_index, libopenjpegextra), Cvoid, (Ptr{Ptr{opj_codestream_index_t}},), p_cstr_index)
end

function opj_get_jp2_metadata(p_codec)
    ccall((:opj_get_jp2_metadata, libopenjpegextra), Ptr{opj_jp2_metadata_t}, (Ptr{opj_codec_t},), p_codec)
end

function opj_get_jp2_index(p_codec)
    ccall((:opj_get_jp2_index, libopenjpegextra), Ptr{opj_jp2_index_t}, (Ptr{opj_codec_t},), p_codec)
end

function opj_set_MCT(parameters, pEncodingMatrix, p_dc_shift, pNbComp)
    ccall((:opj_set_MCT, libopenjpegextra), OPJ_BOOL, (Ptr{opj_cparameters_t}, Ptr{OPJ_FLOAT32}, Ptr{OPJ_INT32}, OPJ_UINT32), parameters, pEncodingMatrix, p_dc_shift, pNbComp)
end

function opj_has_thread_support()
    ccall((:opj_has_thread_support, libopenjpegextra), OPJ_BOOL, ())
end

function opj_get_num_cpus()
    ccall((:opj_get_num_cpus, libopenjpegextra), Cint, ())
end
